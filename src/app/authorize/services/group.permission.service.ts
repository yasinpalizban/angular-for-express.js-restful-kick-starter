...
export class GroupPermissionService extends ApiService<IGroupPermission> implements IApiCommonFunction {
  ...
  nestedUrl(id: string): this {
    this.pageUrl = environment.baseUrl + 'permission/' + id + '/groupPermission';
    this.shared.next(id);
    return this;
  }
  ...
  query(argument?: number | string | object): void {
    ...
    this.subscription.push(super.get(argument).subscribe((gp) => {
      this.dataSubject.next(gp);
    }));
  }
