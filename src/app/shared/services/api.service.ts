...
export class ApiService<T> implements IApiService<T> {
  ...
  protected shared: BehaviorSubject<any>;
  ...
  public get(argument?: number | string | object): Observable<T> {
    ...
    let params: any;
    let queries = '';
    if (typeof argument === 'string' && argument.match(/^[0-9a-fA-F]{24}$/)) {
  ...
  public put(data: any): Observable<T> {
    ...
    if (data instanceof FormData) {
      ...
      return this.httpClient.post<T>(this.pageUrl + '/' + data.get('_id'), data)
        .pipe(catchError(error => this.errorService.handleError(error)));
    } else {
      const id: string = data._id;
      delete data._id;
      return this.httpClient.put<T>(this.pageUrl + '/' + id, data)
        .pipe(catchError(error => this.errorService.handleError(error)));
    }
  ...
  public delete(id: number | string, foreignKey?: number | string): Observable<T> {
    let params;
  ...
  getSharedObservable(): Observable<any> {
    return this.shared.asObservable().pipe(filter(result => !!result));
  }
